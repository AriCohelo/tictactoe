...
const game = (() => {
  ...
  const boardStatus = function () {
    ...
    for (const combo of winningCombos) {
      const [a, b, c] = combo;
      if (cells[a].innerHTML === 'x' && cells[b].innerHTML
        === 'x' && cells[c].innerHTML === 'x') {
        alert('Player 2 Wins!');
        return;
      }
    }
    for (const combo of winningCombos) {
      const [a, b, c] = combo;
      if (cells[a].innerHTML === 'o' && cells[b].innerHTML
        === 'o' && cells[c].innerHTML === 'o') {
        alert('Player 1 Wins!');
        return;
      }
    }
...
const gameboard = (() => {
  const boardCreator = function () {
    ...
    for (let i = 1; i < 10; i++) {
      ...
      boardContainer.appendChild(cell).setAttribute('class', `boardCell cell${[i]}`)
